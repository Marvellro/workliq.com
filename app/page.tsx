    ...
    <main style={{ maxWidth: 680, margin: "0 auto", fontFamily: "system-ui, -apple-system, sans-serif", padding: "0 1.5rem" }}>
      ...
      <nav style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "1.2rem 0 0" }}>
        ...
        <div style={{ display: "flex", gap: 16, alignItems: "center" }}>
          ...
          <span style={{ fontSize: 11, background: "#E6F1FB", color: "#0C447C", border: "0.5px solid #85B7EB", padding: "3px 10px", borderRadius: 20 }}>Early access</span>
        </div>
      </nav>

      {/* HERO */}
      <section style={{ padding: "3.5rem 0 2rem" }}>
        <div style={{ fontSize: 12, color: "#1A56DB", fontFamily: "monospace", letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: "1.2rem", display: "flex", alignItems: "center", gap: 8 }}>
          <span style={{ display: "inline-block", width: 20, height: 1.5, background: "#1A56DB" }} />
          AI workflow automation
        </div>

        <h1 style={{ fontSize: "clamp(32px, 7vw, 48px)", fontWeight: 500, lineHeight: 1.08, letterSpacing: "-0.025em", marginBottom: "1.2rem", color: "#0D0F1A" }}>
          Your workflows,<br />
          <em style={{ fontStyle: "italic", color: "#1A56DB" }}>finally liquid.</em>
        </h1>

        <p style={{ fontSize: 16, color: "#6B7280", lineHeight: 1.65, maxWidth: 480, marginBottom: "2rem" }}>
        ...
        {/* WAITLIST FORM */}
        {!submitted ? (
          <div style={{ background: "#fff", border: "0.5px solid #E5E7EB", borderRadius: 16, padding: "1.25rem", marginBottom: "1rem" }}>
            <form onSubmit={handleSubmit}>
              ...
              <button
                type="submit"
                disabled={loading}
                style={{ width: "100%", padding: "10px", borderRadius: 8, border: "none", background: loading ? "#93AEED" : "#1A56DB", color: "#fff", fontSize: 13, fontWeight: 500, cursor: loading ? "not-allowed" : "pointer", transition: "background 0.15s" }}
              >
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 10, fontSize: 12, color: "#6B7280" }}>
              <div style={{ display: "flex" }}>
                {["#1A56DB", "#6366F1", "#0F6E56", "#E24B4A"].map((bg, i) => (
                  <div key={i} style={{ width: 22, height: 22, borderRadius: "50%", background: bg, border: "1.5px solid #fff", marginLeft: i === 0 ? 0 : -5, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 8, fontWeight: 500, color: "#fff" }}>
                    {["A", "M", "S", "J"][i]}
                  </div>
                ))}
              </div>
          ...
          <div style={{ background: "#fff", border: "0.5px solid #E5E7EB", borderRadius: 16, padding: "2rem", textAlign: "center", marginBottom: "1rem" }}>
            ...
            <div style={{ fontSize: 13, color: "#6B7280", lineHeight: 1.6 }}>
              Check your inbox at <strong>{email}</strong> — we just sent you a confirmation link. Click it to lock in your spot and get your welcome email.
            </div>
          </div>
        )}
      </section>
      ...
      {/* TICKER */}
      <div style={{ overflow: "hidden", borderTop: "0.5px solid #E5E7EB", borderBottom: "0.5px solid #E5E7EB", padding: "9px 0", marginBottom: "2.5rem" }}>
        <div style={{ display: "inline-block", whiteSpace: "nowrap", animation: "ticker 18s linear infinite", fontFamily: "monospace", fontSize: 11, color: "#9CA3AF", letterSpacing: "0.04em" }}>
      ...
      <section style={{ marginBottom: "2.5rem" }}>
        <div style={{ fontSize: 11, fontWeight: 500, color: "#9CA3AF", textTransform: "uppercase", letterSpacing: "0.07em", marginBottom: "1rem" }}>What Workliq does</div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
          {[
            ...
            <div key={i} style={{ background: "#fff", border: "0.5px solid #E5E7EB", borderRadius: 12, padding: 14 }}>
              <div style={{ width: 32, height: 32, background: "#E6F1FB", borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 16, marginBottom: 10 }}>{f.icon}</div>
      ...
      {/* USE CASES */}
      <section style={{ marginBottom: "2.5rem" }}>
        <div style={{ fontSize: 11, fontWeight: 500, color: "#9CA3AF", textTransform: "uppercase", letterSpacing: "0.07em", marginBottom: "1rem" }}>Built for real workflows</div>
        {[
          ...
          <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: 14, padding: "1rem 0", borderBottom: i < 3 ? "0.5px solid #E5E7EB" : "none" }}>
            ...
            <div>
              ...
              <p style={{ fontSize: 12, color: "#6B7280", lineHeight: 1.55, margin: 0 }}>{uc.desc}</p>
            </div>
          </div>
        ))}
      </section>
